...
<?php
    ...
    if(empty($_SESSION['admin'])){
		...
		$limit = 24;
            $pg = mysqli_real_escape_string($config,@$_GET['pg']);
				...
				echo'
                                        ...
                                        <form method="post" action="?page=files&sub=private">
                                        ...
                                        <div class="input-field col s12" style="margin-top:9px">
                                            <select name="filtershare" id="filtershare">';
                                               ...
                                               if($_SESSION['admin']!=1){
                                               echo '<option value="1" selected>File Pribadi</option>
											<option value="2">File Divisi</option>';}
											else {
											echo '<option value="1" selected>Semua File</option>
											<option value="2">Semua File (Tanpa Keterangan)</option>';	
											}
	   ...
	   if($_SESSION['admin']!=1){
	   $query = mysqli_query($config, "SELECT * FROM tbl_file_sharing WHERE divisi='$divisi' AND(file LIKE '%$cari%' AND id_user='$id_user') ORDER BY id DESC LIMIT $curr, $limit");}
	   else {
	   $query = mysqli_query($config, "SELECT * FROM tbl_file_sharing WHERE file LIKE '%$cari%' ORDER BY id DESC LIMIT $curr, $limit");
	   }
        } else {
			if($_SESSION['admin']!=1){
	   $query = mysqli_query($config, "SELECT * FROM tbl_file_sharing WHERE divisi='$divisi' AND id_user='$id_user' ORDER BY id DESC LIMIT $curr, $limit");}
	   else {
