...
$konco=mysqli_query($config,"SELECT * FROM tbl_identitas INNER JOIN tbl_user ON tbl_user.id_user=tbl_identitas.id_user WHERE tbl_identitas.jenis_bank='".$row['kode_bank']."' AND(tbl_identitas.id_user<>8 AND tbl_identitas.id_user<>1 AND tbl_identitas.id_user<>9999 AND tbl_user.status_aktif=1) ORDER BY tbl_user.nip");
