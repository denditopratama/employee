...
$pdf->Cell(103 ,6,'Uraian Potongan',1,0,'C',1);
...
$totalpenerimaan=0;
$ngitung=0;
$konco=mysqli_query($config,"SELECT * FROM tbl_potongan WHERE id_gaji='$id_gaji' GROUP BY kode_potongan");
while($row=mysqli_fetch_array($konco)){
	$hasilbanks=0;
	$jhg=mysqli_query($config,"SELECT uraian FROM tbl_ref_potongan WHERE id='".$row['kode_potongan']."'");
	list($jumlahkeun)=mysqli_fetch_array($jhg);
	$mng=mysqli_query($config,"SELECT COUNT(id_user) FROM tbl_potongan WHERE id_gaji='$id_gaji' AND kode_potongan='".$row['kode_potongan']."'");
	list($jumlahorang)=mysqli_fetch_array($mng);
	$mngs=mysqli_query($config,"SELECT SUM(jumlah) FROM tbl_potongan WHERE id_gaji='$id_gaji' AND kode_potongan='".$row['kode_potongan']."'");
