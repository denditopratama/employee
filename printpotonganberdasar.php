...
$this->Cell(1 ,6,'Asal Instansi : PT Jasamarga Properti',0,1);
...
$this->Cell(30 ,2,'',0,1);
$this->SetFont('Arial','B',9);
$this->Cell(1 ,6,'Asal Instansi : PT Jasa Marga (Persero) Tbk.',0,1);
$this->SetFillColor(176,224,230);
$this->SetFont('Arial','B',9);
...
$ngitung=0;
$jhg=mysqli_query($config,"SELECT tbl_potongan.*,tbl_ref_potongan.uraian,tbl_ref_potongan.jenis_bank,tbl_ref_bank.nama_bank,tbl_ref_potongan.no_rekening,tbl_ref_potongan.atas_nama,tbl_user.status_tugas 
FROM tbl_potongan,tbl_ref_potongan,tbl_ref_bank,tbl_user WHERE tbl_potongan.kode_potongan=tbl_ref_potongan.id AND (tbl_ref_potongan.jenis_bank=tbl_ref_bank.kode_bank AND tbl_potongan.kode_potongan<>28 AND tbl_user.id_user=tbl_potongan.id_user AND tbl_user.status_tugas=2) GROUP BY tbl_potongan.kode_potongan");
while($dat=mysqli_fetch_array($jhg)){
	

    $pdf->Cell(64.5 ,12,''.$dat['uraian'].'',1,0,'L');
    $pdf->Cell(1 ,6,'',0,0);
    $x=$pdf->GetX();
    $y=$pdf->GetY();
    ...
    $totalpenerimaan=0;
$konco=mysqli_query($config,"SELECT tbl_potongan.*,tbl_user.nama,tbl_user.nip FROM tbl_potongan,tbl_user WHERE tbl_potongan.id_gaji='$id_gaji' AND (tbl_user.id_user=tbl_potongan.id_user AND tbl_potongan.kode_potongan<>28 AND tbl_potongan.kode_potongan='".$dat['kode_potongan']."') GROUP BY tbl_potongan.id_user");
while($row=mysqli_fetch_array($konco)){
    $pdf->Cell(6.5 ,6,''.$no++.'',1,0,'C');
    $pdf->Cell(1 ,6,'',0,0);
    $pdf->Cell(11 ,6,''.$row['nip'].'',1,0,'C');
    $pdf->Cell(1 ,6,'',0,0);
    $pdf->Cell(130 ,6,''.$row['nama'].'',1,0,'L');
    $pdf->Cell(1 ,1,'',0,0);
    $pdf->SetFont('Arial','B',9);
    $pdf->Cell(39.5 ,6,''.number_format($row['jumlah'] , 0, ',', '.').'',1,1,'R');
    $pdf->SetFont('Arial','',9);
    $pdf->Cell(1 ,1,'',0,1);
    $totalpenerimaan=$totalpenerimaan+$row['jumlah'];
